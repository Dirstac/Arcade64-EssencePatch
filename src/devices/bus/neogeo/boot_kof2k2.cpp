...
// 修改的 (Gaston90)
/*****************************************************************************/
void neogeo_kf2k2mpd_cart_device::device_add_mconfig(machine_config &config)
{
	NEOBOOT_PROT(config, m_prot);
	NG_CMC_PROT(config, m_cmc_prot);
	NG_PCM2_PROT(config, m_pcm2_prot);
...
void neogeo_kf2k2mp2d_cart_device::device_add_mconfig(machine_config &config)
{
	NEOBOOT_PROT(config, m_prot);
	NG_CMC_PROT(config, m_cmc_prot);
	NG_PCM2_PROT(config, m_pcm2_prot);
...
DEFINE_DEVICE_TYPE(NEOGEO_KF2K2MPD_CART, neogeo_kf2k2mpd_cart_device, "neocart_kf2k2mpd", "Neo Geo KoF 2002 MP Decrypted Cart")
...
neogeo_kf2k2mpd_cart_device::neogeo_kf2k2mpd_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_bootleg_cart_device(mconfig, NEOGEO_KF2K2MPD_CART, tag, owner, clock),
	m_cmc_prot(*this, "cmc_prot"),
	m_pcm2_prot(*this, "pcm2_prot")
{
}
...
DEFINE_DEVICE_TYPE(NEOGEO_KF2K2MP2D_CART, neogeo_kf2k2mp2d_cart_device, "neocart_kf2k2mp2d", "Neo Geo KoF 2002 MP2 Decrypted Cart")
...
neogeo_kf2k2mp2d_cart_device::neogeo_kf2k2mp2d_cart_device(const machine_config &mconfig, const char *tag, device_t *owner, uint32_t clock) :
	neogeo_bootleg_cart_device(mconfig, NEOGEO_KF2K2MP2D_CART, tag, owner, clock),
	m_cmc_prot(*this, "cmc_prot"),
	m_pcm2_prot(*this, "pcm2_prot")
{
}
